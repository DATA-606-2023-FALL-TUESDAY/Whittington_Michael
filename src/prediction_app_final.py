import streamlit as st
import pandas as pd
import numpy as np
from sklearn.ensemble import RandomForestClassifier
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import LabelEncoder
from st_combobox import st_combobox
import pickle

# Step 1: Preprocess the Entire Dataset
@st.cache_data
def load_data():
    data = pd.read_csv('neiss_2022.csv')
    return data

df = load_data()

# Step 2: Train the Model on the Entire Dataset
X = df.drop(['CPSC_Case_Number','Other_Race', 'Body_Part','Diagnosis','Other_Diagnosis', 'Body_Part_2', 'Diagnosis_2', 'Other_Diagnosis_2', 'PSU', 'Stratum', 'Narrative_1','Treatment_Date','Product_2','Product_3','Weight','Disposition'], axis=1)
y_body_part = df['Body_Part']
y_diagnosis = df['Diagnosis']

@st.cache_resource
# Load the pickled model
def load_model(filename):
    with open(filename, 'rb') as file:
        loaded_model = pickle.load(file)
    return loaded_model

# Load the pickled label encoder
def load_label_encoder(filename):
    with open(filename, 'rb') as file:
        label_encoder = pickle.load(file)
    return label_encoder

# Load the pickled model and label encoder
model_body_part = load_model('xgboost_model_body_part.pkl')
model_diagnosis = load_model('xgboost_model_diagnosis.pkl')
le_body_part = load_label_encoder('label_encoder_body_part.pkl')
le_diagnosis = load_label_encoder('label_encoder_diagnosis.pkl')

X_train_body, X_test_body, y_train_body_part, y_test_body_part = train_test_split(X, y_body_part, test_size=0.2, random_state=42)
X_train_diag, X_test_diag, y_train_diagnosis, y_test_diagnosis = train_test_split(X, y_diagnosis, test_size=0.2, random_state=42)

# Step 3: Create the Streamlit App
st.title('NEISS Body Part and Diagnosis Prediction')

age = st.slider('What is your age?', 0, 120, 25)
sex = st.radio('What is your sex?', options=[(0, 'Unknown'), (1, 'Male'),(2, 'Female'),(3,'Non-Binary/Other')])
hispanic = st.radio('Are you Hispanic?', options=[(0, 'Unk/Not stated'), (1, 'Yes'),(2, 'No')])
race = st.selectbox('What is your race?', options=[(0, 'Not Stated'), (1, 'White'), (2, 'Black/African American'), (3, 'Other'), (4, 'Asian'), (5, 'American Indian/Alaska Native'), (6, 'Native Hawaiian/Pacific Islander')])

product_options =[(101,"WASHING MACHINES WITHOUT WRINGERS OR OTHER DRYERS"),
(102,"WRINGER WASHING MACHINES"),
(103,"WASHING MACHINES WITH UNHEATED SPIN DRYERS"),
(106,"ELECTRIC CLOTHES DRYERS WITHOUT WASHERS"),
(107,"GAS CLOTHES DRYERS WITHOUT WASHERS"),
(108,"MANGLE IRONS"),
(110,"ELECTRIC HEATING PADS"),
(112,"SEWING MACHINES OR ACCESSORIES"),
(113,"FLOOR BUFFERS OR WAXERS"),
(114,"RUG SHAMPOOERS"),
(115,"VACUUM CLEANERS"),
(116,"ELECTRIC BROOMS"),
(118,"GAS WATER HEATERS"),
(119,"ELECTRIC WATER HEATERS"),
(125,"WATER SOFTENERS OR CONDITIONERS (APPLIANCES)"),
(126,"WASHING MACHINES, NOT SPECIFIED"),
(127,"CLOTHES DRYERS, NOT SPECIFIED"),
(131,"PROPANE, LP OR BUTANE GAS TANKS OR FITTINGS"),
(132,"ELECTRIC BLANKETS OR SHEETS"),
(133,"OTHER WATER HEATERS"),
(134,"WATER HEATERS, NOT SPECIFIED"),
(135,"WASHER-DRYER COMBINATIONS (WITHIN ONE FRAME)"),
(136,"DRINKING FOUNTAINS"),
(137,"AUTOMATIC DOORS OR DOOR OPENERS"),
(138,"AUTOMATIC GARAGE DOORS OR DOOR OPENERS"),
(139,"INCINERATORS"),
(140,"WASHING MACHINES, OTHER OR NOT SPECIFIED"),
(202,"ELECTRIC COUNTER-MOUNTED COOKING SURFACES"),
(204,"GAS COUNTER-MOUNTED COOKING SURFACES"),
(212,"ELECTRIC CORN POPPERS"),
(213,"ELECTRIC CAN OPENERS"),
(214,"DISHWASHERS"),
(215,"ELECTRIC BLENDERS"),
(216,"ELECTRIC COUNTER-TOP OVENS, BROILERS OR TOASTER OVENS"),
(217,"ELECTRIC COFFEE MAKERS OR TEAPOTS"),
(218,"ELECTRIC OR BATTERY-POWERED KNIVES"),
(219,"ELECTRIC DEEP FRYERS"),
(221,"FOOD WARMERS"),
(222,"ELECTRIC FRYING PANS AND SKILLETS"),
(223,"ELECTRIC GRIDDLES"),
(224,"HOT PLATES"),
(227,"ICE MAKERS (SEPARATE FROM REFRIGERATORS)"),
(231,"ELECTRIC MIXERS"),
(232,"ELECTRIC SCISSORS"),
(234,"TOASTERS"),
(235,"ELECTRIC WAFFLE IRONS"),
(236,"FAUCET WATER HEATERS"),
(237,"GARBAGE DISPOSERS"),
(238,"DRY HEAT IRONS"),
(239,"STEAM IRONS"),
(240,"ELECTRIC KNIFE SHARPENERS"),
(241,"ROTISSERIES"),
(242,"ELECTRIC IMMERSION WATER HEATERS"),
(247,"IRONS, NOT SPECIFIED"),
(250,"RANGE OR OVEN ACCESSORIES"),
(252,"TRASH COMPACTORS"),
(255,"CLOTHES STEAMERS"),
(257,"COUNTER-MOUNTED COOKING SURFACES, NOT SPECIFIED"),
(259,"ELECTRIC RANGES (WITH OVENS)"),
(260,"GAS RANGES (WITH OVENS)"),
(261,"ELECTRIC OVENS"),
(262,"GAS OVENS"),
(263,"FREEZERS (SEPARATE FROM REFRIGERATORS)"),
(264,"MICROWAVE OVENS"),
(265,"OTHER OVENS"),
(266,"OVENS, NOT SPECIFIED"),
(267,"OTHER RANGES (WITH OVENS)"),
(268,"SLOW COOKERS"),
(269,"ELECTRIC KETTLES OR HOT POTS"),
(270,"ICE CRUSHERS"),
(271,"JUICERS"),
(273,"RANGES, NOT SPECIFIED"),
(274,"ICE CREAM MAKERS"),
(275,"FOOD PROCESSORS"),
(276,"REFRIGERATORS"),
(277,"CLOTHING IRONS"),
(278,"ELECTRIC RANGES OR OVENS (EXCL COUNTER-TOP OVENS)"),
(279,"GAS RANGES OR OVENS"),
(280,"OTHER RANGES OR OVENS"),
(281,"RANGES OR OVENS, NOT SPECIFIED"),
(304,"HUMIDIFIERS"),
(305,"VAPORIZERS"),
(306,"DEHUMIDIFIERS"),
(307,"AIR PURIFIERS"),
(308,"BOILERS"),
(309,"COAL FURNACES"),
(310,"GAS FURNACES"),
(311,"OIL FURNACES"),
(312,"ELECTRIC BASEBOARD HEATERS"),
(316,"WOOD BURNING FIREPLACES, FACTORY BUILT"),
(318,"ELECTRIC FURNACES"),
(322,"FURNACES, OTHER OR NOT SPECIFIED"),
(334,"GAS-BURNING FIREPLACES, FACTORY BUILT"),
(336,"BUILT-IN FIREPLACES"),
(340,"THERMOSTATS (FOR HEATING OR COOLING SYSTEMS)"),
(342,"FIREPLACES, NOT SPECIFIED"),
(346,"ELECTRIC FIREPLACES, FACTORY BUILT"),
(348,"PORTABLE ELECTRIC HEATERS"),
(365,"HEAT PUMPS"),
(366,"CATALYTIC HEATERS"),
(367,"COAL OR WOOD-BURNING STOVES"),
(368,"GASOLINE HEATERS (NONCATALYTIC)"),
(369,"OTHER HEATERS OR HEATING SYSTEMS"),
(370,"HEATERS OR HEATING SYSTEMS, NOT SPECIFIED"),
(371,"DUCTWORK FOR HEATING OR COOLING SYSTEMS"),
(372,"BRICK, STONE OR MASONRY CHIMNEYS"),
(373,"METAL CHIMNEYS, STOVEPIPES OR FLUES"),
(374,"PIPES (EXCLUDING SMOKING PIPES)"),
(378,"CHIMNEYS, NOT SPECIFIED"),
(379,"RADIATORS (EXCLUDING VEHICLE RADIATORS)"),
(380,"FANS"),
(381,"AIR CONDITIONERS"),
(384,"FLOOR FURNACES (BUILT INTO FLOOR)"),
(386,"GAS OR LP FLOOR OR WALL HEATERS"),
(388,"ELECTRIC HEATERS, NOT ELSEWHERE CLASSIFIED"),
(389,"GAS OR LP HEATERS, NOT ELSEWHERE CLASSIFIED"),
(390,"HEAT TAPES, ELECTRIC"),
(391,"PORTABLE GAS OR LP HEATERS"),
(392,"GAS OR LP HEATERS, OTHER OR NOT SPECIFIED"),
(393,"HEATERS OR HEATING SYSTEMS, OTHER OR NOT SPECIFIED"),
(394,"OUTDOOR PATIO HEATERS OR FIREPITS (MANUFACTURED)"),
(399,"KEROSENE OR OIL HEATERS"),
(401,"NONELECTRIC CAN OPENERS"),
(405,"UNPOWERED COFFEE MAKERS OR TEAPOTS"),
(408,"IRONING BOARDS OR COVERS"),
(412,"PRESSURE COOKERS OR CANNERS"),
(413,"WASTE CONTAINERS, TRASH BASKETS OR REFUSE BINS"),
(416,"CORKSCREWS"),
(417,"FLATWARE (EXCL. KNIVES)"),
(419,"TIE RACKS, BELT RACKS OR OTHER CLOTHES HANGERS"),
(420,"MANUAL SCISSORS"),
(421,"NONELECTRIC KNIFE SHARPENERS"),
(422,"BOTTLE OPENERS"),
(424,"GLASS DRINKING GLASSES"),
(427,"ICE PICKS"),
(428,"KITCHEN GADGETS, NOT ELSEWHERE CLASSIFIED"),
(429,"FOOD SKEWERS"),
(431,"TOOTHPICKS OR HORS D'OEUVRES PICKS"),
(432,"TRAYS (EXCL. FOOD WARMERS)"),
(434,"DOORSTOPS"),
(435,"KITCHEN MIXING BOWLS, CANISTERS OR SIMILAR CONTAINERS"),
(436,"POTHOLDERS, OVEN MITTS OR HOT PADS"),
(438,"LAUNDRY BASKETS"),
(443,"DRINKING STRAWS"),
(444,"KNIFE SHARPENERS, NOT SPECIFIED"),
(450,"SCISSORS, NOT SPECIFIED"),
(452,"COFFEE MAKERS OR TEAPOTS, NOT SPECIFIED"),
(453,"CAN OPENERS, NOT SPECIFIED"),
(454,"NONGLASS DRINKING GLASSES"),
(459,"MANUAL CLEANING EQUIPMENT"),
(460,"METAL COOKWARE (NONELECTRIC)"),
(461,"NONMETAL COOKWARE (NONELECTRIC)"),
(462,"CHAFING DISHES OR FONDUE POTS"),
(463,"CANDLES, CANDLESTICKS AND OTHER CANDLE HOLDERS"),
(464,"KNIVES, NOT ELSEWHERE CLASSIFIED"),
(465,"OTHER COOKWARE"),
(466,"COOKWARE, NOT SPECIFIED"),
(469,"SLICERS AND CHOPPERS"),
(471,"FOOD GRINDERS"),
(474,"TABLEWARE AND ACCESSORIES"),
(475,"DRINKING GLASSES, NOT SPECIFIED"),
(477,"THERMOMETERS (EXCLUDING MEDICAL THERMOMETERS)"),
(478,"DRINKING GLASSES, CUPS, AND MUGS"),
(480,"MANUAL CLEANING EQUIPMENT (EXCL. BUCKETS AND PAILS)"),
(481,"BREAD MAKING MACHINES"),
(482,"APPLIANCES, OTHER AND NOT SPECIFIED"),
(514,"CERAMICS SUPPLIES OR EQUIPMENT"),
(519,"TELEVISION TABLES OR STANDS"),
(529,"TELESCOPES, BINOCULARS, MICROSCOPES, MAGNIFYING GLASSES"),
(530,"PHONOGRAPH RECORDS"),
(531,"RECORDING TAPES"),
(532,"GLASS BOTTLE CUTTING EQUIPMENT"),
(533,"LAPIDARY EQUIPMENT"),
(536,"PHOTOGRAPHIC EQUIPMENT"),
(545,"AMATEUR TWO-WAY RADIOS"),
(546,"STEREO OR HI-FI COMPONENTS OR ACCESSORIES"),
(547,"SOUND RECORDING OR RECEIVING EQUIPMENT, NOT SPECIFIED"),
(549,"PROJECTORS"),
(550,"TELEPHONES OR TELEPHONE ACCESSORIES"),
(552,"ANTENNAS, NOT SPECIFIED"),
(555,"RADIOS (SELF-CONTAINED UNITS)"),
(556,"TAPE RECORDERS OR PLAYERS (SELF-CONTAINED UNITS)"),
(557,"COMPUTERS (EQUIPMENT AND ELECTRONIC GAMES)"),
(558,"INDOOR ANTENNAS (SEPARATE)"),
(559,"RADIOS, NOT SPECIFIED"),
(561,"OUTDOOR ANTENNAS (SEPARATE)"),
(562,"LASER POINTERS"),
(563,"RECORD PLAYERS (SELF-CONTAINED UNITS)"),
(564,"ART SUPPLIES OR EQUIPMENT"),
(565,"MUSICAL INSTRUMENTS, ELECTRIC OR BATTERY OPERATED"),
(566,"MUSICAL INSTRUMENTS, NOT ELECT. OR BATTERY OR NOT SPEC."),
(567,"COAXIAL CABLE"),
(569,"ART OR CERAMICS SUPPLIES AND EQUIPMENT (EXCL. PAINTBRUSHES)"),
(570,"MUSICAL INSTRUMENTS"),
(571,"BUSINESS AND OFFICE MACHINES"),
(572,"TELEVISIONS"),
(573,"OTHER SOUND RECORDING, REPRODUCING OR RECEIVING EQUIP."),
(574,"REMOTE CONTROL, UNIVERSAL OR NS (EXCL. REMOTES FOR TOYS)"),
(575,"COMPACT DISCS (CDS) AND DVDS"),
(576,"VIDEO PLAYERS AND RECORDERS (EXCL CAMERAS)"),
(601,"BEDSPRINGS OR BEDFRAMES"),
(604,"DESKS, CHESTS, BUREAUS OR BUFFETS"),
(606,"GENERATORS OR POWER PLANTS"),
(609,"GLASS BATHTUB OR SHOWER ENCLOSURES"),
(610,"NONGLASS BATHTUB OR SHOWER ENCLOSURES"),
(611,"BATHTUBS OR SHOWERS"),
(612,"RUNNERS, THROW RUGS OR DOORMATS"),
(613,"ROOM-SIZED, WALL-TO-WALL OR OUTDOOR CARPETING"),
(617,"DRAPERIES, CURTAINS, SHOWER CURTAINS(FABRIC OR PLASTIC)"),
(618,"STEPLADDERS"),
(619,"STRAIGHT LADDERS"),
(620,"STEP STOOLS"),
(623,"BUTANE OR LP GAS METERS"),
(624,"ELECTRIC METERS (UTILITY)"),
(627,"LIGHT BULBS"),
(628,"ELECTRIC CHANDELIERS OR OTHER ATTACHED LIGHT FIXTURES"),
(638,"WINDOW SHADES, VENETIAN BLINDS OR INDOOR SHUTTERS"),
(639,"FLASHLIGHTS OR BATTERY-POWERED LANTERNS"),
(648,"SINKS"),
(649,"TOILETS"),
(650,"SLIPCOVERS"),
(651,"TABLE LINENS"),
(652,"PICTURES, PICTURE FRAMES OR WALL DECORATIONS"),
(653,"ARTIFICIAL FLOWERS OR PLANTS"),
(654,"KNICKKNACKS, STATUES, VASES OR URNS"),
(657,"TOWEL RACKS OR BARS"),
(661,"BUNK BEDS"),
(662,"WATERBEDS OR WATER PILLOWS"),
(663,"FIREPLACE EQUIPMENT"),
(666,"TOWELS OR CLOTHS"),
(667,"BEDSPREADS, THROWS OR COMFORTERS"),
(669,"CLOTHESPINS"),
(670,"RECLINER CHAIR"),
(671,"ROCKING CHAIR"),
(672,"LAUNDRY HAMPERS"),
(676,"RUGS OR CARPETS, NOT SPECIFIED"),
(679,"SOFAS, COUCHES, DAVENPORTS, DIVANS OR STUDIO COUCHES"),
(680,"CONVERTIBLE BEDS, HIDEAWAY BEDS OR SOFA BEDS"),
(684,"UNATTACHED APPLIANCE OR LAMP CORDS"),
(685,"EXTENSION CORDS"),
(687,"BENCHES"),
(688,"EXTENSION LADDERS"),
(689,"BLANKETS, NOT SPECIFIED"),
(692,"MAGAZINE RACKS OR BOOK ENDS"),
(693,"FOOTLOCKERS"),
(694,"BEDS, NOT SPECIFIED"),
(698,"HOT TUBS OR HOME SPAS"),
(699,"FAUCETS OR SPIGOTS"),
(701,"FIRE EXTINGUISHERS"),
(702,"FIRE OR SMOKE ALARMS"),
(704,"BURGLAR ALARMS"),
(706,"LIGHTNING RODS, ARRESTERS OR OTHER GROUNDING DEVICES"),
(707,"PADLOCKS, CHAIN LOCKS OR OTHER LOCKS"),
(709,"SAFES"),
(710,"ELECTRIC TIMERS"),
(711,"FIRE ESCAPE DEVICES"),
(712,"CARBON MONOXIDE DETECTORS"),
(713,"FIRE SPRINKLERS"),
(714,"COMBINATION FIRE/SMOKE ALARM AND CARBON MONOXIDE DETECTORS"),
(803,"POWER SANDERS"),
(804,"POWER ROUTERS"),
(805,"POWER LATHES"),
(807,"POWER JOINTERS"),
(808,"POWER SHAPERS"),
(809,"OTHER PORTABLE OR STATIONARY POWER TOOLS"),
(812,"ELECTRIC WELDING EQUIPMENT"),
(814,"HOISTS, LIFTS, JACKS OR JACK STANDS"),
(815,"ELECTRICAL TESTING EQUIPMENT"),
(819,"ELECTRIC MOTORS (SEPARATE)"),
(820,"INTERNAL COMBUSTION ENGINES (NONVEHICULAR)"),
(821,"AUTOMOTIVE TOOLS OR ACCESSORIES"),
(822,"PAINT SPRAYERS"),
(823,"AIR COMPRESSORS (SEPARATE)"),
(827,"HAMMERS"),
(828,"SCREWDRIVERS"),
(829,"MANUAL DRILLS"),
(830,"HAND SAWS"),
(831,"FUEL-BURNING WELDING EQUIPMENT"),
(832,"PORTABLE CIRCULAR POWER SAWS"),
(833,"WORKSHOP COMPOUNDS OR CHEMICALS"),
(834,"HEAVY-DUTY WORKSHOP STAPLERS"),
(835,"TOOLBOXES OR TOOL KITS"),
(836,"KNIVES WITH REPLACEABLE BLADES"),
(837,"WIRE, NOT SPECIFIED"),
(841,"BENCH OR TABLE SAWS"),
(842,"BAND SAWS"),
(843,"RADIAL ARM SAWS"),
(844,"POWER HACK SAWS"),
(845,"SAWS, NOT SPECIFIED"),
(847,"DRILLS, NOT SPECIFIED"),
(852,"ROPE OR STRING"),
(854,"WORKSHOP FURNISHINGS"),
(855,"PORTABLE POWER DRILLS AND ACCESSORIES"),
(856,"STATIONARY POWER DRILLS AND ACCESSORIES"),
(857,"PLIERS, WIRE CUTTERS OR WRENCHES"),
(858,"REAMERS"),
(859,"SOLDERING EQUIPMENT, NOT SPECIFIED"),
(862,"MANUAL FILING OR SANDING TOOLS"),
(863,"OTHER POWER SAWS"),
(864,"SABRE SAWS"),
(865,"PORTABLE POWER WORKSHOP GRINDERS, BUFFERS, POLISHERS"),
(866,"ELECTRIC SOLDERING EQUIPMENT"),
(867,"WELDING EQUIPMENT, NOT SPECIFIED"),
(868,"NONELECTRIC SOLDERING EQUIPMENT"),
(869,"GLUE GUNS"),
(870,"AWLS, CHISELS OR PLANES"),
(871,"POWER DRILLS OR ACCESSORIES, NOT SPECIFIED"),
(872,"POWER SAWS, NOT SPECIFIED"),
(873,"STATIONARY POWER WORKSHOP GRINDERS, BUFFERS, POLISHERS"),
(874,"CUTTING TORCHES"),
(875,"JIGSAWS"),
(876,"POWER WORKSHOP GRINDERS, BUFFERS, POLISHERS, N.S."),
(877,"TROUBLE LIGHTS OR EXTENSION WORK LIGHTS"),
(878,"LEVELS"),
(879,"VISES OR WORKSHOP CLAMPS"),
(880,"DRAIN SNAKES"),
(881,"OTHER MANUAL WORKSHOP TOOLS"),
(882,"NAIL GUNS OR STUD DRIVERS"),
(883,"BATTERY CHARGERS/ADAPTERS"),
(884,"BATTERIES"),
(885,"CHAINS, NOT ELSEWHERE CLASSIFIED"),
(886,"STRETCH CORDS OR STRETCH STRAPS"),
(887,"PAINTING EQUIP., POWERED (SPRAYERS, ROLLERS, BRUSHES)"),
(888,"SPRAYERS, NOT ELSEWHERE CLASSIFIED"),
(889,"BATTERIES (MOTOR VEHICLE)"),
(890,"TORCHES, NOT SPECIFIED"),
(891,"BATTERIES, NOT SPECIFIED"),
(892,"NON-MOTOR VEHICLE BATTERIES"),
(893,"TOOLS, NOT SPECIFIED"),
(894,"HACKSAWS"),
(895,"POWER SAWS, OTHER OR NOT SPECIFIED"),
(896,"WELDING EQUIPMENT"),
(897,"POWER WORKSHOP GRINDERS, BUFFERS OR POLISHERS"),
(898,"MANUAL WORKSHOP GRINDERS, BUFFERS OR POLISHERS"),
(899,"WORKSHOP GRINDERS, BUFFERS OR POLISHERS, N.S."),
(904,"FLOOR WAXES"),
(905,"FURNITURE POLISHES OR WAXES"),
(908,"PAINT THINNERS OR VARNISH THINNERS"),
(909,"ADHESIVES"),
(910,"GASOLINE"),
(913,"LUBRICANTS"),
(914,"DRY ICE"),
(915,"METHYL ALCOHOL (SOLVENT)"),
(917,"CHARCOAL"),
(920,"CAULKING OR SPACKLING COMPOUNDS"),
(921,"CHEMICALS, NOT ELSEWHERE CLASSIFIED"),
(925,"PAINTBRUSHES OR ROLLERS, MANUAL"),
(926,"PHOTOGRAPHIC CHEMICALS"),
(927,"TAPES (WITH ADHESIVE SURFACES)"),
(929,"DRAIN CLEANERS"),
(930,"AMMONIA, HOUSEHOLD"),
(931,"METAL POLISHES, TARNISH REMOVERS OR PREVENTATIVES"),
(932,"SHOE POLISHES"),
(933,"TURPENTINE"),
(934,"DISHWASHER DETERGENTS"),
(936,"ACIDS"),
(937,"RUST REMOVERS"),
(938,"SWIMMING POOL CHEMICALS"),
(940,"LIGHTER FLUIDS"),
(941,"FUELS FOR CHAFING DISHES OR FONDUE POTS"),
(942,"OVEN CLEANERS"),
(943,"MOTH REPELLENTS"),
(945,"PINE OIL CLEANING AND DISINFECTANT PREPARATIONS"),
(949,"LAUNDRY SOAPS OR DETERGENTS"),
(951,"TOILET BOWL PRODUCTS"),
(952,"FABRIC TREATMENT PRODUCTS"),
(953,"ABRASIVE CLEANERS"),
(954,"GENERAL PURPOSE HOUSEHOLD CLEANERS"),
(955,"AUTOMOTIVE CHEMICALS"),
(956,"BLEACHES (NONCOSMETIC)"),
(957,"DYES (NONCOSMETIC)"),
(958,"OTHER POLISHES"),
(959,"POLISHES, NOT SPECIFIED"),
(960,"PAINTS, VARNISHES OR SHELLACS"),
(961,"RUST PREVENTATIVES"),
(962,"DIESEL FUELS"),
(963,"KEROSENE"),
(964,"LAMP OILS"),
(965,"FUEL OILS, NOT SPECIFIED"),
(966,"ANTIFREEZE"),
(967,"WINDSHIELD WIPER FLUIDS"),
(968,"CAUSTICS"),
(969,"LYE"),
(970,"WALLPAPER CLEANERS (CHEMICAL)"),
(971,"WALLPAPER STEAMERS"),
(972,"PAINT OR VARNISH REMOVERS, PAINTBRUSH CLEANERS"),
(973,"LIQUID ROOM DEODORIZERS OR FRESHENERS"),
(974,"SOLID ROOM DEODORIZERS OR FRESHENERS"),
(975,"ROOM DEODORIZERS OR FRESHENERS, NOT SPECIFIED"),
(976,"DETERGENTS, NOT SPECIFIED"),
(977,"SPOT REMOVERS OR CLEANING FLUIDS"),
(978,"AUTOMOTIVE WAXES, POLISHES OR CLEANERS"),
(979,"DISHWASHING LIQUID"),
(980,"FUEL STORAGE TANKS"),
(981,"GASOLINE CANS"),
(982,"SCHOOL LABORATORY CHEMICALS"),
(983,"SOAPS (EXCL. LAUNDRY SOAPS OR DETERGENTS)"),
(984,"SOLVENTS, OTHER OR NOT SPECIFIED"),
(1001,"ELECTRIC FENCES"),
(1002,"HOME PASTEURIZERS"),
(1003,"CREAM SEPARATORS"),
(1004,"FARM TRACTORS"),
(1005,"POWER TAKE-OFFS"),
(1006,"FARM TILLAGE EQUIPMENT"),
(1009,"COMBINES OR THRESHING MACHINES"),
(1011,"FARM ELEVATORS OR CONVEYORS"),
(1012,"FARM WAGONS"),
(1014,"FARM SPRAYERS"),
(1015,"IRRIGATION EQUIPMENT"),
(1016,"FEED GRINDERS, MIXERS OR BLENDERS"),
(1017,"MILKING MACHINES"),
(1018,"TOW CHAINS, CABLES OR HEAVY-DUTY CHAINS"),
(1019,"POST-HOLE AUGERS"),
(1021,"FARM MOWERS"),
(1022,"TILLAGE EQUIPMENT, NOT SPECIFIED"),
(1023,"FERTILIZERS, NOT SPECIFIED"),
(1025,"ENDGATE SEEDERS"),
(1026,"OTHER CROP PROCESSING EQUIPMENT"),
(1030,"FORAGE HARVESTERS"),
(1036,"MANURE SPREADERS"),
(1037,"PUMP JACKS"),
(1038,"POST-HOLE DRIVERS"),
(1039,"FERTILIZER DISTRIBUTORS (GRANULAR)"),
(1040,"RAKES (FARM MACHINERY)"),
(1042,"WINDROWERS OR SWATHERS"),
(1043,"CROP CHOPPERS"),
(1044,"FORK LIFTS OR LIFT TRUCKS"),
(1052,"STANCHIONS"),
(1053,"FERTILIZERS AND OTHER CHEMICALS FOR OUTDOOR USE"),
(1054,"OTHER PLANTING EQUIPMENT"),
(1055,"HAY PROCESSING EQUIPMENT"),
(1056,"TOBACCO STRINGERS, HARVESTERS OR SLEDS"),
(1057,"CROP PICKERS"),
(1058,"CORN PLANTERS OR GRAIN DRILLS"),
(1059,"SILO LOADERS AND UNLOADERS, SCREW OR SWEEP AUGERS"),
(1060,"PLANTING EQUIPMENT, NOT SPECIFIED"),
(1061,"CROP PROCESSING EQUIPMENT, NOT SPECIFIED"),
(1062,"TRACTORS, OTHER OR NOT SPECIFIED, N.O.S."),
(1102,"VACUUM CONTAINERS"),
(1103,"SELF-CONTAINED OPENERS"),
(1107,"OTHER CONTAINERS"),
(1112,"METAL CONTAINERS"),
(1114,"CARDBOARD PRODUCTS"),
(1116,"CONTAINERS WITH KEY OPENERS"),
(1120,"GLASS SOFT DRINK BOTTLES"),
(1122,"GLASS ALCOHOLIC BEVERAGE BOTTLES"),
(1123,"PLASTIC CONTAINERS (RIGID OR SEMI-RIGID)"),
(1124,"GLASS TUBING OR TEST TUBES"),
(1125,"WOODEN CONTAINERS"),
(1127,"NONGLASS BOTTLES OR JARS"),
(1128,"PAPER BAGS"),
(1130,"PLASTIC BAGS"),
(1131,"PLASTIC WRAPPING PRODUCTS"),
(1132,"PLASTIC PRODUCTS, NOT SPECIFIED"),
(1133,"AEROSOL CONTAINERS"),
(1134,"OTHER GLASS BOTTLES OR JARS"),
(1135,"BOTTLES OR JARS, NOT SPECIFIED"),
(1136,"CANNING JARS"),
(1137,"PAPER PRODUCTS"),
(1138,"PRESSURIZED CONTAINERS (NONAEROSOL)"),
(1139,"ALUMINUM FOIL WRAPPING PRODUCTS"),
(1140,"GLASS BOTTLES OR JARS, NOT SPECIFIED"),
(1141,"CONTAINERS, NOT SPECIFIED"),
(1142,"BAGS (EXCLUDING PAPER OR PLASTIC BAGS)"),
(1143,"BUCKETS OR PAILS"),
(1144,"BAGS, NOT ELSEWHERE CLASSIFIED"),
(1145,"OTHER SPECIFIED PLASTIC PRODUCTS"),
(1200,"SPORTS AND RECREATIONAL ACTIVITY, N.E.C."),
(1202,"BICYCLES OR ACCESSORIES"),
(1204,"BASEBALL, ACTIVITY AND RELATED EQUIPMENT"),
(1205,"BASKETBALL, ACTIVITY AND RELATED EQUIPMENT"),
(1206,"BOWLING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1207,"BOXING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1208,"CROQUET (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1211,"FOOTBALL (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1212,"GOLF (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1213,"GOLF CARTS, MOTORIZED VEHICLE"),
(1215,"LACROSSE (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1217,"SLEDS"),
(1232,"ELECTRIC CHARCOAL LIGHTERS"),
(1233,"TRAMPOLINES"),
(1234,"TREEHOUSES OR PLAYHOUSES"),
(1235,"ARCHERY (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1237,"GAS, AIR OR SPRING-OPERATED GUNS"),
(1239,"HORSEBACK RIDING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1240,"AQUARIUMS OR ACCESSORIES"),
(1242,"SLIDES OR SLIDING BOARDS"),
(1243,"SEE-SAWS OR TEETER BOARDS"),
(1244,"MONKEY BARS OR OTHER PLAYGROUND CLIMBING APPARATUS"),
(1246,"WADING POOLS"),
(1247,"FUEL CHARCOAL LIGHTERS"),
(1248,"DARTS (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1255,"ALCOHOL-FUELED HEATERS"),
(1257,"HORSESHOES (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1258,"MOUNTAIN CLIMBING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1260,"BILLIARDS OR POOL (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1261,"SURFING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1264,"WATER SKIING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1266,"VOLLEYBALL (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1267,"SOCCER (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1268,"TRACK AND FIELD (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1269,"TABLE TENNIS (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1270,"WRESTLING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1271,"BATONS"),
(1272,"GYMNASTICS AND ASSOCIATED EQUIPMENT"),
(1273,"TOBOGGANS"),
(1274,"SNOW DISKS"),
(1275,"SCUBA DIVING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1276,"TETHERBALL (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1277,"SWIMMING POOL SLIDES"),
(1278,"DIVING OR DIVING BOARDS"),
(1279,"ICE HOCKEY (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1282,"HANDBALL (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1283,"UNICYCLES"),
(1284,"SWIMMING POOLS, NOT SPECIFIED"),
(1285,"CHARCOAL LIGHTERS, NOT SPECIFIED"),
(1290,"SNOWMOBILES (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1293,"AMUSEMENT ATTRACTIONS (INCLUDING RIDES)"),
(1294,"BLEACHERS"),
(1295,"FIELD HOCKEY (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1299,"SNOW TUBING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(1301,"TRICYCLES"),
(1306,"GASOLINE- OR OTHER FUEL-POWERED MODELS"),
(1309,"KITES OR KITE STRING"),
(1310,"POGO STICKS"),
(1313,"FIREWORKS"),
(1314,"ROCKETRY SETS"),
(1317,"FLYING TOYS"),
(1318,"UNPOWERED MODELS OR PARTS"),
(1319,"METAL OR PLASTIC MOLDING SETS"),
(1322,"CHILDREN'S PLAY TENTS, PLAY TUNNELS OR OTHER ENCLOSURES"),
(1325,"INFLATABLE TOYS"),
(1326,"BLOCKS, STACKING TOYS OR PULL TOYS"),
(1327,"NONWHEELED RIDING TOYS, UNPOWERED"),
(1328,"WAGONS (CHILDREN'S)"),
(1329,"SCOOTERS, UNPOWERED"),
(1330,"POWERED RIDING TOYS"),
(1332,"ELECTRIC TRAINS OR ACCESSORIES"),
(1333,"SKATEBOARDS, UNPOWERED OR UNSPECIFIED"),
(1334,"ELECTRIC TOY OVENS"),
(1335,"ELECTRIC TOY IRONS"),
(1338,"TOY BOWS OR ARROWS"),
(1342,"COSTUMES OR MASKS"),
(1344,"TOY MUSICAL INSTRUMENTS"),
(1345,"BUILDING SETS"),
(1346,"CLACKER BALLS"),
(1347,"BALLOONS (TOY)"),
(1349,"STILTS"),
(1350,"SQUEEZE OR SQUEAKER TOYS"),
(1352,"SLINGSHOTS OR SLING-PROPELLED TOYS"),
(1353,"TOY BOXES OR CHESTS"),
(1354,"MARBLES"),
(1356,"ENGINE FUELS FOR MODELS"),
(1362,"WOODBURNING KITS"),
(1365,"WATER TOYS"),
(1376,"MOLDING COMPOUNDS"),
(1381,"TOYS, NOT ELSEWHERE CLASSIFIED"),
(1383,"ELECTRIC TOY RACING CARS OR ACCESSORIES"),
(1389,"OTHER TOY WEAPONS (NONPROJECTILE)"),
(1390,"TOY GUNS, NOT SPECIFIED"),
(1392,"TOY SPORTS EQUIPMENT"),
(1393,"CHEMISTRY SETS OR SCIENCE KITS"),
(1394,"DOLLS, PLUSH TOYS, AND ACTION FIGURES"),
(1395,"TOYS, NOT SPECIFIED"),
(1398,"WHEELED RIDING TOYS, UNPOWERED"),
(1399,"TOY GUNS WITH PROJECTILES"),
(1401,"POWER LAWN MOWERS, NOT SPECIFIED"),
(1402,"MANUAL PUSH MOWERS (UNPOWERED)"),
(1403,"OTHER UNPOWERED GARDEN TOOLS"),
(1404,"POWER LAWN TRIMMERS OR EDGERS"),
(1405,"GARDEN TRACTORS"),
(1406,"SNOW THROWERS, BLOWERS"),
(1407,"GARDEN SPRAYERS"),
(1408,"POWER TILLERS OR CULTIVATORS"),
(1409,"OTHER POWER GARDEN TOOLS"),
(1411,"CHAIN SAWS"),
(1413,"GREENHOUSE OR GARDENING SUPPLIES"),
(1414,"GARDEN HOSES, NOZZLES OR SPRINKLERS"),
(1415,"MANUAL SNOW OR ICE REMOVAL TOOLS"),
(1417,"DECORATIVE YARD EQUIPMENT"),
(1422,"RIDING POWER LAWN MOWERS"),
(1425,"WHEELBARROWS OR LAWN CARTS"),
(1426,"HATCHETS OR AXES"),
(1427,"POWER HEDGE TRIMMERS"),
(1430,"SEED AND FERTILIZER SPREADERS"),
(1431,"GRASS AND LEAF CATCHERS (ATTACHMENTS FOR LAWN MOWERS)"),
(1432,"ANIMAL TRAPS"),
(1433,"POWER LEAF MULCHERS AND GRINDERS"),
(1434,"SNOW THROWERS OR BLOWERS (ATTACHMENTS FOR LAWN MOWERS)"),
(1435,"SNOW PLOWS"),
(1439,"LAWN MOWERS, NOT SPECIFIED"),
(1441,"LAWN VACUUMS"),
(1445,"PLANT FOODS OR FERTILIZERS FOR POTTED PLANTS"),
(1446,"REEL POWER LAWN MOWERS"),
(1447,"PRUNING OR TRIMMING EQUIPMENT, NOT SPECIFIED"),
(1448,"ROTARY POWER LAWN MOWERS"),
(1449,"MANUAL HEDGE TRIMMERS"),
(1450,"MANUAL LAWN TRIMMERS OR EDGERS"),
(1451,"SEEDS"),
(1452,"OUTDOOR ELECTRIC LIGHTING EQUIPMENT"),
(1453,"OTHER MANUAL PRUNING OR TRIMMING EQUIPMENT"),
(1454,"OTHER POWER PRUNING OR TRIMMING EQUIPMENT"),
(1455,"MANUAL PRUNING OR TRIMMING EQUIPMENT, NOT SPECIFIED"),
(1456,"POWER PRUNING OR TRIMMING EQUIPMENT, NOT SPECIFIED"),
(1457,"LOG SPLITTERS"),
(1458,"SNOW THROWERS OR BLOWERS (SELF-CONTAINED UNITS)"),
(1459,"SNOW THROWERS OR BLOWERS, NOT SPECIFIED"),
(1460,"INSECT TRAPS, POWERED"),
(1461,"LEAF BLOWERS"),
(1462,"PUMPS, NOT ELSEWHERE CLASSIFIED"),
(1463,"BRUSHCUTTERS, POWERED"),
(1464,"LAWN TRIMMERS OR EDGERS, POWERED (EXCL. BRUSHCUTTERS)"),
(1465,"DECORATIVE YARD EQUIPMENT, EXCL WATER FEATURES"),
(1466,"DECORATIVE WATER FEATURES, INCL MAN-MADE PONDS & FOUNTAINS"),
(1502,"BABY CHANGING TABLES"),
(1505,"BABY CARRIAGES"),
(1506,"BABY GATES OR BARRIERS"),
(1508,"BABY WALKERS OR JUMPERS"),
(1509,"BABY BOTTLES OR NIPPLES"),
(1510,"BOTTLE WARMERS"),
(1511,"STERILIZERS (HOME USE)"),
(1512,"DIAPERS"),
(1513,"PLAYPENS"),
(1515,"BABY SCALES"),
(1517,"BABY RATTLES"),
(1518,"YOUTH CHAIRS"),
(1519,"CAR SEATS (FOR INFANTS OR CHILDREN)"),
(1520,"BABY EXERCISERS"),
(1522,"BABY STROLLERS"),
(1524,"BABY HARNESSES"),
(1525,"PACIFIERS OR TEETHING RINGS"),
(1526,"CRIB MOBILES OR CRIB GYMS"),
(1527,"BABY CARRIERS OR SLINGS (BACKPACKS)"),
(1528,"DIAPER PAILS"),
(1529,"PORTABLE CRIBS"),
(1531,"BABY CARRIERS (BICYCLE-MOUNTED)"),
(1533,"NIGHT-LIGHTS"),
(1534,"CAR BEDS FOR INFANTS"),
(1535,"POTTY CHAIRS OR TRAINING SEATS"),
(1536,"CRIB EXTENDER RAILS OR YOUTH BED RAILS"),
(1537,"BASSINETS OR CRADLES"),
(1539,"SHOELACE FASTENERS (FOR INFANTS)"),
(1542,"BABY MATTRESSES OR PADS"),
(1543,"NONPORTABLE CRIBS"),
(1544,"BABY BATHS OR BATHINETTES"),
(1545,"CRIBS, NOT SPECIFIED"),
(1548,"BABY CARRIERS, NOT SPECIFIED"),
(1549,"OTHER BABY CARRIERS"),
(1550,"INFANT & TODDLER PLAY CTRS, EXCL JUMPERS,BOUNCERS&EXERCISERS"),
(1551,"DIAPER FASTENERS"),
(1552,"CRIBS, NONPORTABLE OR NOT SPECIFIED"),
(1553,"PORTABLE BABY SWINGS (FOR HOME USE)"),
(1554,"SAFETY PINS"),
(1555,"HIGH CHAIRS"),
(1556,"ATTACHED HIGHCHAIR"),
(1557,"BABY BATHTUB SEATS OR RINGS (NOT TOYS)"),
(1558,"BABY BOUNCER SEATS (EXCL. JUMPERS)"),
(1560,"BABY SLINGS AND WRAPS"),
(1561,"FRAMED BABY CARRIERS (CARRIED ON THE BACK)"),
(1562,"OTHER SOFT BABY CARRIERS"),
(1597,"NON-POISONING DEATH OF VICTIM UNDER AGE 5 - NO PRODUCT"),
(1598,"OVERLAY DEATHS"),
(1599,"INFANT DEATH"),
(1602,"HAIR DRYERS"),
(1604,"CIGARETTE OR PIPE LIGHTERS"),
(1605,"WIGS, FALLS OR HAIRPIECES"),
(1606,"EYEGLASSES"),
(1607,"EYE PROTECTION DEVICES"),
(1608,"POWERED TOOTHBRUSHES OR ORAL IRRIGATORS"),
(1609,"SUN LAMPS"),
(1610,"MASSAGE DEVICES OR VIBRATORS"),
(1612,"SAUNAS"),
(1613,"ELECTRIC SHOE POLISHERS"),
(1615,"FOOTWEAR"),
(1616,"JEWELRY"),
(1617,"EAR PROTECTION DEVICES"),
(1618,"RESPIRATORY PROTECTION DEVICES"),
(1619,"PERSONAL PROTECTION DEVICES"),
(1620,"HEARING AIDS"),
(1622,"WATCHES"),
(1623,"LUGGAGE"),
(1624,"CONTACT LENSES"),
(1625,"LIGHTED MAKE-UP MIRRORS"),
(1629,"NONELECTRIC TOOTHBRUSHES"),
(1630,"PAPER MONEY OR COINS"),
(1634,"HEAT OR INFRARED LAMPS"),
(1635,"DENTURES OR FALSE TEETH"),
(1636,"ORTHODONTIC BRACES"),
(1637,"ELECTRIC COMBS"),
(1638,"COMBS OR HAIRBRUSHES, UNPOWERED"),
(1641,"CLOTHESBRUSHES"),
(1643,"KEYS, KEY RINGS OR KEY CHAINS"),
(1644,"NIGHTWEAR"),
(1645,"DAY WEAR"),
(1646,"OUTERWEAR"),
(1647,"CLOTHING ACCESSORIES"),
(1650,"DESK SUPPLIES"),
(1651,"COMBS, NOT SPECIFIED"),
(1654,"PAPER CUTTERS"),
(1658,"CLOTHING, NOT SPECIFIED"),
(1659,"MANICURE, PEDICURE, AND MAKE-UP BRUSHES/TOOLS"),
(1660,"UMBRELLAS"),
(1661,"NONELECTRIC RAZORS OR SHAVERS"),
(1662,"HAIR GROOMING EQUIPMENT OR ACCESSORIES, N.S."),
(1664,"ELECTRIC RAZORS OR SHAVERS"),
(1666,"TEETH CLEANING DEVICES, NOT SPECIFIED"),
(1667,"RAZORS OR SHAVERS, NOT SPECIFIED"),
(1669,"PINS AND NEEDLES"),
(1671,"SEWING BASKET ARTICLES"),
(1674,"HARD CONTACT LENSES"),
(1675,"SOFT CONTACT LENSES"),
(1676,"CONTACT LENSES, NOT SPECIFIED"),
(1677,"OTHER CLOTHING"),
(1678,"BATH OR FACIAL CLEANSING BRUSHES"),
(1679,"GROCERY OR SHOPPING CARTS, UNPOWERED"),
(1680,"DOLLIES, HANDTRUCKS OR LUGGAGE CARRIERS"),
(1681,"SUNTAN BOOTHS"),
(1682,"HAIR CURLERS, CURLING IRONS, CLIPS & HAIRPINS"),
(1683,"HAIR CLIPPERS AND TRIMMERS"),
(1684,"CARTS, OTHER OR NOT SPECIFIED"),
(1685,"PENS AND PENCILS"),
(1686,"COINS"),
(1687,"LIGHTERS, NOT SPECIFIED"),
(1688,"FUEL MULTI-PURPOSE LIGHTERS, INCL. MICRO-TORCHES"),
(1689,"ELECTRIC MULTI-PURPOSE LIGHTERS"),
(1690,"MULTI-PURPOSE LIGHTERS, NOT SPECIFIED"),
(1701,"ARTIFICIAL CHRISTMAS TREES"),
(1705,"ASHTRAYS"),
(1706,"CRUTCHES, CANES OR WALKERS"),
(1707,"WHEELCHAIRS"),
(1708,"SPECIAL BEDS OR ORTHOPEDIC BEDS"),
(1710,"FIRST AID EQUIPMENT"),
(1711,"CHRISTMAS TREE LIGHTS"),
(1712,"CHRISTMAS TREE STANDS OR SUPPORTS"),
(1714,"SEASONAL DECORATIONS"),
(1715,"PET SUPPLIES"),
(1716,"HYPODERMIC NEEDLES OR SYRINGES"),
(1717,"ORTHOPEDIC BRACES"),
(1718,"FLARES"),
(1719,"LUNCH BOXES OR PAILS"),
(1720,"PARTY FAVORS"),
(1724,"LASERS"),
(1725,"CATHODE-RAY TUBES"),
(1726,"LOCKERS"),
(1727,"TRAFFIC CONTROL DEVICES"),
(1728,"MATCHBOOKS"),
(1729,"CHRISTMAS DECORATIONS (NONELECTRIC)"),
(1730,"WOODEN MATCHES"),
(1731,"MATCHES, NOT SPECIFIED"),
(1732,"TOMBSTONES OR GRAVE MARKERS"),
(1733,"VENDING MACHINES"),
(1734,"MUSIC BOXES OR CHIMES"),
(1735,"COIN-OPERATED CAR-WASHING EQUIPMENT"),
(1736,"ELECTRIC CHRISTMAS DECORATIONS (EXCL. TREE LIGHTS)"),
(1737,"HOUSEPLANTS"),
(1738,"PUBLIC USE ITEMS, NOT ELSEWHERE CLASSIFIED"),
(1739,"ASHTRAYS, ELECTRIC OR BATTERY OPERATED"),
(1740,"ASHTRAYS, NOT ELEC. OR BATTERY OR NOT SPECIFIED"),
(1741,"TARPAULINS"),
(1742,"HEATING OR COOLING COMFORT PACKS (NONELECTRIC)"),
(1743,"THERMOMETERS, MEDICAL"),
(1744,"MOTORIZED VEHICLES, NEC (3 OR MORE WHEELS)"),
(1803,"THERMAL OR SOUND INSULATION MATERIALS"),
(1807,"FLOORS OR FLOORING MATERIALS"),
(1808,"OUTDOOR AWNINGS OR SHUTTERS"),
(1811,"BRICKS OR CONCRETE BLOCKS (NOT PART OF STRUCTURE)"),
(1812,"GUTTERS, DRAINPIPES, DOWNSPOUTS OR RUN-OFF PIPES"),
(1814,"ELEVATORS OR OTHER LIFTS"),
(1816,"SCAFFOLDING"),
(1817,"PORCHES, BALCONIES, OPEN-SIDE FLOORS OR FLOOR OPENINGS"),
(1818,"CISTERNS, CESSPOOLS OR SEPTIC TANKS"),
(1819,"NAILS, SCREWS, CARPET TACKS OR THUMBTACKS"),
(1820,"CABINET OR DOOR HARDWARE"),
(1821,"CLOTHESLINES OR CLOTHES DRYING RACKS"),
(1823,"STORM DOORS WITH GLASS PANELS"),
(1825,"SLIDING GLASS DOORS OR PANELS"),
(1826,"STORM WINDOWS"),
(1827,"SCREEN DOORS"),
(1828,"WINDOW SCREENS"),
(1829,"HANDRAILS, RAILINGS OR BANISTERS"),
(1830,"SEPARATE GARAGES OR TOOL SHEDS"),
(1836,"JALOUSIE GLASS WINDOWS"),
(1838,"LUMBER, BOARDS OR PANEL PIECES (NOT PART OF STRUCTURE)"),
(1840,"PULL-DOWN OR FOLDING STAIRS"),
(1841,"MOBILE HOMES"),
(1842,"STAIRS OR STEPS"),
(1843,"RAMPS OR LANDINGS"),
(1845,"PANEL DOORS (FLUSH OR SLAB)"),
(1849,"DOORS, NOT SPECIFIED"),
(1854,"PLASTIC PANELS FOR DOORS OR WINDOWS"),
(1855,"PLASTIC PRODUCTS, NOT ELSEWHERE CLASSIFIED"),
(1857,"ARTIFICIAL TURF"),
(1858,"STORM DOORS WITHOUT GLASS PANELS"),
(1863,"WALLPAPER"),
(1864,"COUNTERS OR COUNTERTOPS"),
(1865,"POLES"),
(1866,"GENERAL HOME OR ROOM INVOLVEMENT IN FIRES"),
(1867,"FIXED FLOOR-LENGTH GLASS PANELS"),
(1870,"WINDOW SILLS OR FRAMES"),
(1871,"FENCES OR FENCE POSTS"),
(1873,"WINDOWS OR WINDOW GLASS, NOT SPECIFIED"),
(1875,"OTHER WINDOWS OR WINDOW GLASS"),
(1876,"HOUSE REPAIR OR CONSTRUCTION MATERIALS,NEC"),
(1878,"DOOR SILLS OR FRAMES"),
(1879,"OTHER NONGLASS DOORS (NONGARAGE)"),
(1880,"NONGLASS DOORS (NONGARAGE), NOT SPECIFIED"),
(1882,"OTHER GLASS DOORS"),
(1883,"GLASS DOORS, NOT SPECIFIED"),
(1884,"CEILINGS AND WALLS (INTERIOR PART OF COMPLETED STRUCTURE)"),
(1885,"WOOD PANELING AND PARTICLEBOARD"),
(1886,"GARAGE DOORS (EXCL. AUTOMATIC GARAGE DOORS OR OPENERS)"),
(1887,"TOOL SHEDS"),
(1888,"WINDOW OR DOOR SECURITY BARRIERS"),
(1889,"ELEVATORS OR OTHER LIFTS (EXCL. ESCALATORS, HOISTS, JACKS,"),
(1890,"ESCALATORS"),
(1891,"MOVING WALKS"),
(1892,"GLASS DOORS OR DOORS WITH GLASS PANELS"),
(1893,"DOORS, OTHER OR NOT SPECIFIED"),
(1894,"WINDOWS AND WINDOW GLASS, OTHER THAN STORM WINDOWS"),
(1895,"TREE STANDS (HUNTING)"),
(1898,"GAS FUMES OR GAS VAPORS OF UNKNOWN ORIGIN"),
(1899,"CARBON MONOXIDE POISONING (WHEN SOURCE IS UNKNOWN"),
(1901,"MOTOR VEHICLES OR PARTS (LICENSED, FOUR OR MORE WHEELS)"),
(1902,"TRAINS"),
(1903,"ALCOHOL (BEVERAGE)"),
(1906,"AIRCRAFT"),
(1909,"CIGARETTES, CIGARS, PIPES OR TOBACCO"),
(1910,"LICENSED TWO-WHEELED MOTOR VEHICLES"),
(1913,"COSMETICS, INCL FACIAL & EYE MAKE-UP, DEODORANTS, ETC."),
(1914,"VETERINARY MEDICINES"),
(1915,"LINIMENTS OR RUBBING COMPOUNDS"),
(1916,"PREPARATIONS CONTAINING IRON SALTS"),
(1923,"ASPIRIN OR ASPIRIN COMPOUNDS"),
(1926,"PESTICIDES"),
(1927,"LIQUID DRUGS"),
(1928,"ANTIHISTAMINES"),
(1929,"DRUGS OR MEDICATIONS, NOT SPECIFIED"),
(1930,"ASPIRIN SUBSTITUTES"),
(1931,"TABLET OR CAPSULE DRUGS"),
(1932,"OTHER DRUGS OR MEDICATIONS"),
(1933,"FOODS (EXCLUDING HOT WATER)"),
(1934,"HOT WATER"),
(1935,"GUNPOWDER OR AMMUNITION"),
(1936,"BB'S OR PELLETS"),
(1937,"AIR BAGS, DEPLOYED (FOR NHTSA STUDY)"),
(1939,"ILLEGAL DRUGS (INCL COCAINE, CRACK, HEROIN, MARIJ, PCP...)"),
(1940,"MOUTHWASHES OR MOUTH RINSES (EXCLUDING PEROXIDE)"),
(2001,"ANIMAL INDUCED INJURY"),
(2002,"PLANTS, TREES, AND SHRUBS"),
(2003,"INSECT INDUCED INJURIES"),
(2200,"INDUSTRIAL EQUIPMENT"),
(2288,"NO PRODUCT INVOLVED (OCCUPATIONAL INJURIES ONLY)"),
(2300,"SCHOOL EQUIPMENT"),
(2400,"MEDICAL EQUIPMENT"),
(2401,"RADIOGRAPHIC EQUIPMENT"),
(2402,"FLUOROSCOPIC EQUIPMENT"),
(2403,"THERAPEUTIC EQUIPMENT"),
(2404,"GERMICIDAL LAMPS"),
(2405,"DIATHERMY EQUIPMENT (SHORT WAVE, MICROWAVE, ULTRASONIC)"),
(2406,"BRACING OR SUPPORTING DEVICES"),
(2407,"BATHS (MEDICAL EQUIPMENT)"),
(2408,"CATHETERS"),
(2409,"CONTRACEPTIVE DEVICES, INCLUDING IUD'S"),
(2415,"DENTAL PADS OR RELINERS"),
(2420,"EAR PIERCING DEVICES"),
(2423,"HEART VALVES, ARTIFICIAL"),
(2425,"TRACTION EQUIPMENT (HOME USE)"),
(2429,"INVITRO DIAGNOSTIC KITS"),
(2430,"KIDNEY DIALYSIS MACHINES"),
(2431,"INTRAOCULAR DEVICES"),
(2435,"OXYGEN ADMINISTRATION APPARATUS"),
(2437,"PACEMAKERS, CARDIAC"),
(2439,"STIMULATORS, ELECTRICAL"),
(2440,"SUPPORT GARMENTS AND STOCKINGS, MEDICAL"),
(2443,"STERILIZERS, MEDICAL"),
(2447,"RADIATION THERAPY EQUIPMENT"),
(2448,"OSTOMY APPLIANCES"),
(2450,"ANESTHESIOLOGICAL DEVICES, NOT ELSEWHERE CLASSIFIED"),
(2452,"CARDIOVASCULAR DEVICES, NOT ELSEWHERE CLASSIFIED"),
(2454,"DENTAL DEVICES, NOT ELSEWHERE CLASSIFIED"),
(2456,"EAR, NOSE AND THROAT DEVICES, NOT ELSEWHERE CLASSIFIED"),
(2458,"GASTROENTEROLOGICAL OR UROLOGICAL DEVICES"),
(2460,"GENERAL OR PLASTIC SURGERY DEVICES"),
(2462,"OTHER HOSPITAL OR PERSONAL USE MEDICAL DEVICES"),
(2464,"NEUROLOGICAL DEVICES, NOT ELSEWHERE CLASSIFIED"),
(2466,"OBSTETRICAL OR GYNECOLOGICAL DEVICES"),
(2468,"OPHTHALMIC DEVICES, NOT ELSEWHERE CLASSIFIED"),
(2470,"PHYSIATRIC DEVICES, NOT ELSEWHERE CLASSIFIED"),
(2472,"RADIOLOGICAL DEVICES, NOT ELSEWHERE CLASSIFIED"),
(2474,"ARTIFICIAL LIMBS"),
(2475,"ATOMIZING DEVICES"),
(2476,"MONITORING SYSTEMS, MEDICAL"),
(2477,"MEDICAL GLOVES"),
(2479,"PROSTHETIC DEVICES, OTHER"),
(2480,"RESPIRATORY DEVICES, NOT ELSEWHERE CLASSIFIED"),
(2500,"ANTIPERSPIRANTS AND DEODORANTS (UNDERARM)"),
(2510,"BATH PRODUCTS"),
(2511,"BUBBLE BATHS"),
(2520,"DEPILATORIES OR HAIR REMOVERS"),
(2531,"EYE MASCARAS"),
(2532,"EYELINERS"),
(2533,"OTHER EYE MAKE-UP PREPARATIONS"),
(2534,"EYE MAKE-UP PREPARATIONS, NOT SPECIFIED"),
(2540,"FACE CARE PREPARATIONS"),
(2541,"FACE HORMONE CREAMS"),
(2550,"FRAGRANCE PREPARATIONS"),
(2560,"HAND AND BODY CREAMS OR LOTIONS"),
(2571,"HAIR COLORING SHAMPOOS"),
(2572,"HAIR DYES (PERMANENT DARK TONES)"),
(2573,"HAIR LIGHTENERS WITH COLOR (PERMANENT BLOND TONES)"),
(2574,"HAIR COLORING PREPARATIONS, NOT SPECIFIED"),
(2575,"OTHER HAIR COLORING PREPARATIONS"),
(2580,"HAIR GROOMING PREPARATIONS"),
(2590,"NONCOLORING HAIR SHAMPOOS"),
(2600,"HAIR SPRAYS"),
(2610,"HAIR WAVING PREPARATIONS OR STRAIGHTENERS"),
(2621,"MAKE-UP PREPARATIONS"),
(2630,"NAIL PREPARATIONS"),
(2631,"NAIL HARDENERS"),
(2640,"ORAL HYGIENE PRODUCTS"),
(2650,"GENITAL AREA PRODUCTS"),
(2660,"POWDER PREPARATIONS"),
(2670,"SHAVING PREPARATIONS"),
(2680,"SOAPS"),
(2690,"SUNTAN OR SUNSCREEN PREPARATIONS"),
(3200,"WATER TUBING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3202,"PEDAL-POWERED ADULT VEHICLES (THREE OR MORE WHEELS)"),
(3203,"SKEET SHOOTING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3211,"GROUND WATER SLIDES"),
(3214,"CAMPING EQUIPMENT"),
(3215,"MOPEDS OR POWER-ASSISTED CYCLES"),
(3216,"ROLLER SKATING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3217,"SKATING (ACTIVITY, APPAREL OR EQUIPMENT) NOT SPECIFIED"),
(3218,"CHARCOAL OR WOOD-BURNING GRILLS"),
(3219,"OTHER PLAYGROUND EQUIPMENT"),
(3220,"VOLLEYBALL (ACTIVITY, APPAREL OR EQUIPMENT), N.S."),
(3221,"ABOVE-GROUND SWIMMING POOLS (EXCL PORTABLE POOLS)"),
(3222,"BADMINTON (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3223,"FISHING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3224,"OTHER GUNS OR FIREARMS"),
(3225,"ORGANIZED SOCCER (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3226,"BASKETBALL (ACTIVITY, APPAREL OR EQUIPMENT), N.S."),
(3227,"INFORMAL FOOTBALL (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3228,"ORGANIZED BASEBALL (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3229,"ELECTRIC GRILLS"),
(3230,"KEROSENE GRILLS OR STOVES"),
(3231,"GYMNASTICS (ACTIVITY, APPAREL OR EQUIPMENT), N.S."),
(3233,"OTHER GRILLS OR STOVES"),
(3234,"RUGBY (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3235,"OTHER BALL SPORTS (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3236,"BALL SPORTS (ACTIVITY, APPAREL OR EQUIPMENT), N.S."),
(3237,"ORGANIZED GYMNASTICS (ACTIVITY, APPAREL OR EQUIP.)"),
(3240,"INFORMAL BASEBALL (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3241,"INFORMAL SOCCER (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3242,"ORGANIZED BASKETBALL (ACTIVITY, APPAREL OR EQUIP.)"),
(3243,"ORGANIZED VOLLEYBALL (ACTIVITY, APPAREL OR EQUIP.)"),
(3244,"FOOTBALL (ACTIVITY, APPAREL OR EQUIPMENT), N.S."),
(3245,"STREET HOCKEY (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3246,"SWINGS OR SWING SETS"),
(3247,"ICE OR SNOW BOATING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3248,"GAS OR LP GRILLS OR STOVES (FOR OUTDOOR USE)"),
(3249,"GRILLS, NOT SPECIFIED"),
(3250,"PORTABLE FOOD OR BEVERAGE COOLERS"),
(3251,"BUILT-IN SWIMMING POOLS"),
(3252,"SLEEPING BAGS"),
(3253,"GUNS, NOT SPECIFIED"),
(3254,"CHEERLEADING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3255,"ICE SKATING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3256,"SQUASH, RACQUET BALL OR PADDLE BALL"),
(3257,"MARTIAL ARTS (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3258,"MINIBIKES OR TRAIL BIKES"),
(3259,"GO-CARTS"),
(3260,"FENCING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3261,"SHUFFLEBOARD (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3262,"SWIMMING POOL EQUIPMENT"),
(3263,"INFORMAL GYMNASTICS (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3265,"WEIGHT LIFTING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3267,"INFORMAL BASKETBALL (ACTIVITY, APPAREL OR EQUIP.)"),
(3268,"INFORMAL VOLLEYBALL (ACTIVITY, APPAREL OR EQUIP.)"),
(3269,"ORGANIZED FOOTBALL (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3270,"BASEBALL (ACTIVITY, APPAREL OR EQUIP.), NOT SPECIFIED"),
(3271,"SOCCER (ACTIVITY, APPAREL OR EQUIPMENT), NOT SPECIFIED"),
(3272,"HOCKEY (ACTIVITY, APPAREL OR EQUIPMENT), NOT SPECIFIED"),
(3273,"PLAYGROUND EQUIPMENT, NOT SPECIFIED"),
(3274,"SWIMMING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3276,"WATER POLO (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3277,"EXERCISE EQUIPMENT"),
(3278,"DANCING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3279,"FLOTATION TOYS (EXCL OFFICIAL LIFE-SAVING DEVICES)"),
(3282,"CURLING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3283,"SNOW SKIING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3284,"TENNIS (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3285,"ALL TERRAIN VEHICLES ATV (THREE WHEELS/OFF-ROAD ONLY)"),
(3286,"ALL TERRAIN VEHICLES ATV (FOUR WHEELS/OFF ROAD ONLY)"),
(3287,"ALL TERRAIN VEHICLES ATV (# OF WHEELS UNSPECIFIED/OFF ROAD)"),
(3288,"DUNE BUGGIES/BEACH BUGGIES"),
(3289,"DARTS, FOR INDOOR USE (ACTIVITY OR EQUIPMENT)"),
(3290,"DARTS, LAWN (ACTIVITY OR EQUIPMENT)"),
(3291,"DARTS, NOT OTHERWISE SPECIFIED (N.O.S.)"),
(3292,"PERSONAL WATERCRAFT, POWERED"),
(3293,"WATER SLIDES, NOT ELSEWHERE CLASSIFIED"),
(3294,"WATER SLIDES, BACKYARD/HOME (NOT SWIMMING POOL SLIDES)"),
(3295,"WATER SLIDES, PUBLIC (STATIONARY AMUSEMENT RIDES)"),
(3296,"ALL TERR. VEH. ATV (MORE THAN 4 WHEELS; EXCLUSIVELY OFF-RD."),
(3297,"IN-LINE SKATING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3298,"BOATS AND BOATING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(3299,"EXERCISE (ACTIVITY OR APPAREL, W/O EQUIP)"),
(4002,"BEDDING, NOT SPECIFIED"),
(4003,"STOOLS, NOT SPECIFIED"),
(4004,"MIRRORS OR MIRROR GLASS"),
(4005,"FILTERS"),
(4008,"NONELECTRIC BLANKETS"),
(4009,"NONBABY MATTRESSES"),
(4010,"MATTRESSES, NOT SPECIFIED"),
(4011,"INFLATABLE FURNITURE"),
(4013,"OTHER FURNITURE"),
(4014,"FURNITURE, NOT SPECIFIED"),
(4015,"UPHOLSTERED FOOTSTOOLS, OTTOMANS OR HASSOCKS"),
(4016,"BEACH CHAIRS OR FOLDING CHAIRS"),
(4017,"NONUPHOLSTERED RECLINER CHAIRS"),
(4018,"NONUPHOLSTERED ROCKING CHAIRS"),
(4019,"UPHOLSTERED RECLINER CHAIRS"),
(4020,"UPHOLSTERED ROCKING CHAIRS"),
(4021,"OTHER CHAIRS"),
(4022,"CHAIRS, NOT SPECIFIED"),
(4023,"RECLINER CHAIRS, NOT SPECIFIED"),
(4024,"ROCKING CHAIRS, NOT SPECIFIED"),
(4025,"BARSTOOLS OR KITCHEN STOOLS"),
(4026,"OTHER STOOLS"),
(4030,"BATHTUB OR SHOWER ENCLOSURES, NOT SPECIFIED"),
(4032,"OTHER LADDERS"),
(4033,"LADDERS, NOT SPECIFIED"),
(4036,"ELECTRIC TABLE LAMPS, FLOOR LAMPS OR DESK LAMPS"),
(4039,"OTHER ELECTRIC LIGHTING EQUIPMENT"),
(4041,"ELECTRIC LIGHTING EQUIPMENT, NOT SPECIFIED"),
(4042,"LIGHTING EQUIPMENT, NOT SPECIFIED"),
(4043,"CLOCKS"),
(4045,"FABRICS (NOT PART OF FINISHED ITEM)"),
(4047,"BOOKS, MAGAZINES, ALBUMS OR SCRAPBOOKS"),
(4050,"PILLOWS"),
(4051,"SHEETS OR PILLOWCASES"),
(4052,"OTHER UPHOLSTERED CHAIRS"),
(4053,"UPHOLSTERED CHAIRS, NOT SPECIFIED"),
(4054,"OTHER BEDDING"),
(4055,"DRAPERY OR CURTAIN RODS, HOOKS OR RINGS"),
(4056,"CABINETS, RACKS, ROOM DIVIDERS AND SHELVES, NEC"),
(4057,"TABLES (EXCL.  BABY CHANGING TABLES, BILLIARD OR POOL TABLES"),
(4058,"BARS AND BAR ACCESSORIES, NOT ELSEWHERE CLASSIFIED"),
(4059,"SCALES  (EXCL. BABY SCALES)"),
(4060,"FUEL-BURNING LIGHTING EQUIPMENT"),
(4061,"ELECTRIC OUTLETS OR RECEPTACLES"),
(4062,"ELECTRIC WIRE OR WIRING SYSTEMS (EXCL PANELBOARDS RECEPT"),
(4063,"PANELBOARDS, CIRCUIT BREAKERS, GFCI'S, OR FUSES"),
(4064,"FUTONS"),
(4065,"CLOCKS, ELECTRIC OR BATTERY OPERATED"),
(4066,"SCALES, ELECTRIC/BATTERY OPERATED (EXCLUD. BABY SCALES)"),
(4067,"CLOCKS, NOT ELEC. OR BATTERY OR NOT SPECIFIED"),
(4068,"SCALES, NOT ELEC. OR BATTERY OR NOT SPECIFIED"),
(4069,"COTS"),
(4070,"FLOOR LAMPS OR TABLE LAMPS, HALOGEN"),
(4071,"FLOOR LAMPS OR TABLE LAMPS, ELECTRIC (EXCL HALOGEN)"),
(4072,"BEDSPRINGS OR BEDFRAMES EXCL BUNK BEDS & ADULT BED RAILS"),
(4073,"ADULT BED RAILS"),
(4074,"CHAIRS, OTHER OR NOT SPECIFIED"),
(4075,"BED RAILS"),
(4076,"BEDS OR BEDFRAMES, OTHER OR NOT SPECIFIED"),
(4077,"EXTENSION OR STRAIGHT LADDERS"),
(4078,"LADDERS, OTHER OR NOT SPECIFIED"),
(4079,"FOOTSTOOLS, OTTOMANS OR HASSOCKS"),
(4080,"STOOLS, OTHER OR NOT SPECIFIED"),
(4081,"CORDS, ELECTRICAL, OTHER OR NOT SPECIFIED"),
(4082,"TODDLER BEDS"),
(4083,"SURGE SUPPRESSORS OR POWER STRIPS"),
(4084,"RESIDENTIAL ALTERNATIVE ENERGY SYSTEMS FOR ON-SITE POWER GEN"),
(4802,"STRAPS, NON-STRETCH, OTHER, OR NOT SPECIFIED"),
(5001,"OTHER TOY WEAPONS (PROJECTILE)"),
(5004,"TOYS, NOT ELSEWHERE CLASSIFIED"),
(5005,"RIDING TOYS, NOT SPECIFIED"),
(5006,"OTHER TOY GUNS"),
(5007,"TOY WEAPONS, NOT SPECIFIED"),
(5010,"CRAYONS OR CHALK"),
(5011,"BOOK BAGS OR BACK CARRIERS (EXC. BABY CARR. LUGGAGE, CAM"),
(5013,"TOY MAKE-UP KITS OR COSMETICS"),
(5015,"TOY CAPS, CAP TOYS OR CAP GUNS"),
(5016,"BALLS, OTHER OR NOT SPECIFIED"),
(5017,"FLYING DISCS AND BOOMERANGS"),
(5018,"DOLL HOUSES AND OTHER PLAY SCENES"),
(5019,"GAMES OR GAME PARTS (EXCL MARBLES & COMPUTER GAMES)"),
(5020,"PRETEND ELECTRONICS, TOOLS, HOUSEWARES & APPLIANCES"),
(5021,"TOY VEHICLES (EXCLUDING RIDING TOYS)"),
(5022,"SCOOTERS, POWERED"),
(5023,"SCOOTERS, UNPOWERED"),
(5024,"SCOOTERS, UNSPECIFIED"),
(5025,"HOVERBOARDS AND POWERED SKATEBOARDS"),
(5029,"CAMPING EQUIPMENT (EXCL. TRAILERS, LIGHTING EQUIPMENT,"),
(5030,"TRACK & FIELD (ACT., APPAREL, EQP.; EX.  JOG, RUN FIT)"),
(5031,"SNOWBOARDING (ACTIVITY, APPAREL OR EQUIPMENT)"),
(5032,"ROLLER HOCKEY (ACTIVITY, APPAREL OR EQUIPMENT)"),
(5033,"MOUNTAIN OR ALL-TERRAIN BICYCLES AND ACCESSORIES"),
(5034,"SOFTBALL (ACTIVITY, APPAREL OR EQUIPMENT)"),
(5035,"MINIBIKES, POWERED"),
(5036,"TWO-WHEELED, POWERED, OFF-ROAD VEHICLES"),
(5037,"HAMMOCKS"),
(5038,"PERSONAL FITNESS/ACTIVITY TRACKERS"),
(5040,"BICYCLES AND ACCESSORIES, (EXCL.MOUNTAIN OR ALL-TERRAIN)"),
(5041,"BASEBALL (ACTIVITY, APPAREL OR EQUIPMENT; EXCL SOFTBALL)"),
(5042,"SCOOTERS / SKATEBOARDS, POWERED"),
(5043,"PORTABLE SWIMMING POOLS"),
(5044,"UTILITY VEHICLES UTV"),
(5555,"< 5 POISONINGS - NO OTHER CODE"),
(6666,"INHALED/ABUSED SUBSTANCE"),
(7001,"YARD / HOME IMPROVEMENT EQUIP - CONSUMER USE, NO OTHER CODE"),
(7777,"OTHER PRODUCT"),
(7878,"NO PRODUCT INVOLVED / PRODUCT NOT LISTED"),
(8888,"NO PRODUCT"),
(9000,"ASSIGNED CASE WITH OUT-OF-SCOPE PRODUCT"),
(9001,"MENTAL HEALTH SERVICES STUDY"),
(9002,"SEXUAL VIOLENCE"),
(9003,"NONSEXUAL INTIMATE PARTNER VIOLENCE"),
(9100,"OUT OF SCOPE PRODUCT - RETAILER REPORT"),
(9200,"DRYWALL CONTROL - NO COMPLAINT"),
(9999,"UNCATEGORIZED PRODUCT")
]


#def format_product(item):
    # This function will return the product name for display
    #return item[1] if item else ""

#def search_products(searchterm: str):
    #if searchterm:
        #return [(code, name) for code, name in product_options if searchterm.lower() in name.lower()]
    #else:
        #return product_options

product_1_selection = st.selectbox(  # Use the defined function for formatting
    placeholder="Type to search for products...",
    label="What product caused your injury?",
    options=product_options,
)

selected_product_code = product_1_selection[0] if product_1_selection else None
location = st.selectbox('Where did this injury occur?', options=[(0, 'Unknown'), (1, 'Home'),(2, 'Farm'),(3, 'Apartment'),(4, 'Street'),(5, 'Public'),(6, 'Mobile'),(7, 'Industry'),(8, 'School'),(9, 'Sports')])
fire = st.radio('Was fire involved?', options=[(0, 'No or Unknown'), (1, 'Yes, Fire Department was Involved'),(2, 'Yes, Fire Department was not Involved'),(3,'Yes, Unknown if Fire Department was Involved')])
alcohol = st.radio('Was alcohol involved?',options=[(0, 'No or Unknown'), (1, 'Yes')])
drugs = st.radio('Were drugs involved?', options=[(0, 'No or Unknown'), (1, 'Yes')])

top_3_body_parts = None
top_3_diagnoses = None

# Step 4: Map the predictions
body_part_mapping = {
    0: 'INTERNAL',
    30: 'SHOULDER',
    31: 'UPPER TRUNK',
    32: 'ELBOW',
    33: 'LOWER ARM',
    34: 'WRIST',
    35: 'KNEE',
    36: 'LOWER LEG',
    37: 'ANKLE',
    38: 'PUBIC REGION',
    75: 'HEAD',
    76: 'FACE',
    77: 'EYEBALL',
    78: 'UPPER TRUNK(OLD)',
    79: 'LOWER TRUNK',
    80: 'UPPER ARM',
    81: 'UPPER LEG',
    82: 'HAND',
    83: 'FOOT',
    84: '25-50% OF BODY',
    85: 'ALL PARTS BODY',
    86: 'OTHER(OLD)',
    87: 'NOT STATED/UNK',
    88: 'MOUTH',
    89: 'NECK',
    90: 'LOWER ARM(OLD)',
    91: 'LOWER LEG(OLD)',
    92: 'FINGER',
    93: 'TOE',
    94: 'EAR'
}

diagnosis_mapping = {
    41: 'INGESTION',
    42: 'ASPIRATION',
    46: 'BURN, ELECTRICAL',
    47: 'BURN, NOT SPEC.',
    48: 'BURN, SCALD',
    49: 'BURN, CHEMICAL',
    50: 'AMPUTATION',
    51: 'BURNS, THERMAL',
    52: 'CONCUSSION',
    53: 'CONTUSIONS, ABR.',
    54: 'CRUSHING',
    55: 'DISLOCATION',
    56: 'FOREIGN BODY',
    57: 'FRACTURE',
    58: 'HEMATOMA',
    59: 'LACERATION',
    60: 'DENTAL INJURY',
    61: 'NERVE DAMAGE',
    62: 'INTERNAL INJURY',
    63: 'PUNCTURE',
    64: 'STRAIN, SPRAIN',
    65: 'ANOXIA',
    66: 'HEMORRHAGE',
    67: 'ELECTRIC SHOCK',
    68: 'POISONING',
    69: 'SUBMERSION',
    70: 'OTHER',
    71: 'OTHER',
    72: 'AVULSION',
    73: 'RADIATION',
    74: 'DERMA/CONJUNCT'
}

body_part_classes = model_body_part.classes_
diagnosis_classes = model_diagnosis.classes_

# Step 5: Make Predictions
if st.button('Predict'):
    input_df = pd.DataFrame(data={
        'Age': [age],
        'Sex': [sex[0]],
        'Race': [race[0]],
        'Hispanic':[hispanic[0]],
        'Location':[location[0]],
        'Fire_Involvement':[fire[0]],
        'Product_1':[selected_product_code],
        'Alcohol':[alcohol[0]],
        'Drug':[drugs[0]]
    })
    
    # Get probabilities for each class
    body_part_probs = model_body_part.predict_proba(input_df)
    diagnosis_probs = model_diagnosis.predict_proba(input_df)    

    # Get indices of top 3 predictions for body part and diagnosis
    top_3_body_part_indices = np.argsort(body_part_probs[0])[-3:][::-1]
    top_3_diagnosis_indices = np.argsort(diagnosis_probs[0])[-3:][::-1]

    # Map indices to actual classes
    top_3_body_parts = [le_body_part.inverse_transform([i])[0] for i in top_3_body_part_indices]
    top_3_diagnoses = [le_diagnosis.inverse_transform([i])[0] for i in top_3_diagnosis_indices]

# Step 6: Output the Results
if top_3_body_parts is not None and top_3_diagnoses is not None:
    st.write('Top 3 Predicted Body Parts Injured:')
    for part in top_3_body_parts:
        st.write(f'- {body_part_mapping.get(part, "Unknown Body Part")}')

    st.write('Top 3 Predicted Diagnoses:')
    for diag in top_3_diagnoses:
        st.write(f'- {diagnosis_mapping.get(diag, "Unknown Diagnosis")}')